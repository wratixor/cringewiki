...
В этой серверной версии статьи создаются через веб-интерфейс и хранятся в
SQLite. Клонирование репозитория устанавливает движок и его документацию, но
не импортирует статьи из Markdown-файлов: Git-импорт — отдельная будущая
функция. При любом запуске создаются системные точки «Главная»,
«Пользователи», «Теги» и «Рикролл»; «Главная» всегда является стартовой
страницей и ведёт к остальным трём.
