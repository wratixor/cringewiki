...
3. Не запускайте `python -m server.seed` на публичном сервере: он создаёт
   ...
   системные точки «Пользователи», «Теги» и «Рикролл» создаются сами, а
   начальная страница и справка импортируются из публичной папки `wiki/`.
...
- перед обновлением сделайте резервную копию, затем выполните
  ...
  `sudo systemctl restart cringewiki`. Перезапуск автоматически импортирует
  Git-статьи из `wiki/`, пользовательские посты в SQLite не меняются;
